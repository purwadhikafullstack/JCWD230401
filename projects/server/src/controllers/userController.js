                ...
                if (checkpw && getuser[0].dataValues.isSuspended == 0) {
                    ...
                    if (getuser[0].dataValues.attempts < 5) {
                        ...
                        res.status(400).send({
                            success: false,
                            message:
                                "Account suspended ❌ please reset your password",
                        });
                    }
                }
            } else {
            ...
            if (checkExistingUser == 0) {
                if (req.files.length == 1) {
                    if (req.body.password == req.body.confirmationPassword) {
                        ...
                        console.log(
                            "Check data after hash password :",
                            req.body
                        ); //testing purposes
                        const uuid = uuidv4();
                        const { name, email, password, phone } = req.body;
    ...
    verify: async (req, res, next) => {
        try {
            ...
            if (!checkverifieduser[0].dataValues.isVerified) {
                ...
                console.log(
                    "ini isi otp dari getotp:",
                    getotp[0].dataValues.otp
                );
                const { otp } = req.body;
